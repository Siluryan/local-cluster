...
O ficheiro `infraestructure/cluster/main.tf` foi ajustado para usar **exclusivamente `VM.Standard.A1.Flex` (Ampere)** nos três computes criados pelo módulo, somando **4 OCPUs** e **24 GB** de RAM — alinhado ao limite típico **Always Free** da OCI para Ampere na conta (confirmar na documentação oficial da época; cotas podem mudar).

| Componente | Shape | Recursos | Função |
|------------|--------|----------|--------|
| Worker (pool `np1`) | VM.Standard.A1.Flex | **2 OCPU**, **12 GB** RAM, boot **50 GB** | Único nó do Kubernetes |
| Bastion | VM.Standard.A1.Flex | **1 OCPU**, **6 GB** RAM, boot **50 GB** | Salto SSH até ao operator (exigido pelo módulo Oracle) |
...
| **Object Storage** | Bucket de state (`bootstrap`) — armazenamento e pedidos; tipicamente **baixo** para ficheiros de state. |
| **Imagens de contentores** | Workloads **amd64** podem precisar de emulação ou rebuild para **ARM** nos nodes A1; não é custo direto da OCI, mas pode exigir mais recursos ou troubleshooting. |
...
Helm, `LoadBalancer`, PVCs, registos externos e túneis podem acrescentar custos **independentes** do `cluster/main.tf`. Consulte a secção de workloads na calculadora.

---

## 5. Boas práticas

1. Ativar **budgets** e alertas na consola OCI.  
